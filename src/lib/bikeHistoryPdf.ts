...
/**
 * Garmin attribution for the exported document.
 *
 * An export is downstream data under the Garmin API Brand Guidelines, which
 * require the attribution to travel with the data and carry the trademark
 * notice. Ideally it repeats per page; expo-print's render engines do not
 * honor fixed positioning reliably (see the footer note in the stylesheet),
 * so this sits with the end-of-document signature like the rest of the
 * footer. Rendered only when Garmin actually contributed rides, since the
 * guidelines equally forbid the mark where its data is absent.
 */
function buildGarminAttribution(bike: HistoryBike): string {
  if (!bike.contributingSources?.includes('garmin')) return '';
  return `<footer class="attribution">
  <span>${escapeHtml(GARMIN_INSIGHT_ATTRIBUTION)}</span>
  <span>${escapeHtml(GARMIN_TRADEMARK_NOTICE)}</span>
</footer>`;
}
...
export function buildBikeHistoryHtml(params: {
  ...
  const garminAttributionHtml = buildGarminAttribution(bike);
...
<style>
  * { box-sizing: border-box; }
   ...
   */
  footer.attribution { display: block; margin-top: 8px; padding-top: 8px; border-top: none; font-size: 6.5pt; line-height: 1.4; }
  footer.attribution span { display: block; }
